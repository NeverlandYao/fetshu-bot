...
class CozeMessage(BaseModel):
    ...
    type: Literal["question", "answer", "function_call", "tool_output", "follow_up"] = (
...
class CozeWorkflowChatRequest(BaseModel):
    ...
    additional_messages: List[CozeMessage] = Field(
        default_factory=list, description="附加消息"
    )
...
class CozeWorkflowResponse(BaseModel):
    ...
    events: List[CozeWorkflowEvent] = Field(
